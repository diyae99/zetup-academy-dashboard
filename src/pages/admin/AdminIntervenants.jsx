...
import { createIntervenantAccount } from '../../services/adminUsers';
...
export default function AdminIntervenants() {
  ...
  const rows = useMemo(() => data.intervenants.filter((i) => i.name.toLowerCase().includes(search.toLowerCase()) || i.email.includes(search)), [data.intervenants, search]);
  const counts = (id, key) => key === 'groups' ? data.groups.filter((g) => g.intervenantId === id).length : data[key].filter((x) => (x.createdBy || x.intervenantId) === id).length;
  ...
  const columns = [
    ...
    { key: 'groups', label: 'Groupes', render: (r) => counts(r.id, 'groups') },
    { key: 'quizzes', label: 'Quizzes', render: (r) => counts(r.id, 'quizzes') },
    { key: 'resources', label: 'Resources', render: (r) => counts(r.id, 'resources') },
    ...
    { key: 'actions', label: 'Actions', render: (r) => <AccountActions row={r} account={accountFor(r.id)} onShow={() => showCredentials(r)} onReset={() => resetPassword(r)} onToggle={() => toggleStatus(r)} /> },
  ];
